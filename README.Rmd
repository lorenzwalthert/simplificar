...
If visualizations should be written to files, just use the 
corresponding low-level transformer (i.e. `vis_1d_distr_to_file`)
instead.
...
position (via `...`). Here, we can use `readr::parse_factor()` for safe factor 
parsing.
...
vector in the list column `aes` of the gg table (see below). The same is true 
