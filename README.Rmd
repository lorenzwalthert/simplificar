...
* Low-level interface: The functions `vis_[n]d_[*]()`, e.g. `vis_1d_distr()`. The 
...
* high-level interface: The function `vis_cols()` let's you plot various plots 
  ...
  low-level interface introduced in the first bullet, e.g. `vis_1_distr()`. 
...
Note that in case a visualization has multiple columns, each of them is stored
in a separate vector in the list column `aes`, The same is true for the class
attribute. This might be helpful for filtering (see below).
...
As shown above, if you supply more arguments to `vis_cols()` than the indicated 
...
Below, we add a mean line to plot 2 and 3.
```{r}
mtcars %>%
    vis_cols(vs, contains("hp"), "cyl", transformer = vis_2d_point) %>%
